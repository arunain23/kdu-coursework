...
const useStyles = createUseStyles({
  ...
  clicked: {
    ...
    "&:hover": {
      ...
      backgroundColor: "red",
      backgroundImage:
        "url(\"data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='18' height='auto' fill='white' viewBox='0 0 24 24'><path d='M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z'/><path d='M0 0h24v24H0z' fill='none'/></svg>\")",
      backgroundRepeat: "no-repeat",
      backgroundPosition: "center",
    },
  },
...
export function Explore() {
  ...
  const filteredStocks = useSelector(
    (state: RootState) => state.stocksLoad.filteredStocks
  );
  ...
  const toggleFavorite = (stock: Stock) => {
    const isFavorite = filteredStocks.find(
      (filtstock) => filtstock.stock_name === stock.stock_name
    );
    if (isFavorite) {
      const updatedFavorites = filteredStocks.filter(
        (filtstock) => filtstock.stock_name !== stock.stock_name
      );
  ...
  const totalNumberOfPages = Math.ceil(stocks.length / stocksPerPage);
    ...
    <div>
      ...
      <div className={classes.MainBox}>
        <div className={classes.Title}>
          <h3 className={classes.nameDetail}>Companys</h3>
        ...
        {currentStocks.map((stock) => (
          <div className={classes.showstocks} key={stock.stock_name}>
            <Link
              to={`/stock-details/${stock.stock_name}`}
              style={{ width: "60%", textDecoration: "none", color: "black" }}
            >
              <div className="nameDetail">{stock.stock_name}</div>
            </Link>
            ...
            >
              <button
                className={
                  filteredStocks.some(
                    (filteredStock) =>
                      filteredStock.stock_name === stock.stock_name
                  )
                    ? classes.clicked
                    : classes.unclicked
                }
