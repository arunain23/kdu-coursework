...
export function StockDetails() {
  ...
  const handleBuy = () => {
    if (quantityRef.current && quantityRef.current.value !== "") {
      const totalPrice = generate * parseInt(quantityRef.current.value);
      console.log(totalPrice);
      if (totalPrice <= walletBalance) {
        setWalletBalance((prevBalance) => prevBalance - totalPrice);
        ...
        const newMessage = (
          <div>
            {quantityRef.current.value} stocks{" "}
            <span style={{ color: "green", paddingLeft: "130px" }}>Buy</span>{" "}
            <br />
  ...
  const handleSell = () => {
    if (quantityRef.current && quantityRef.current.value !== "") {
      ...
      const newMessage = (
        <div>
          {quantityRef.current.value} stocks{" "}
          <span style={{ color: "Red", paddingLeft: "130px" }}>Sell</span>{" "}
          <br />
          <span style={dateTimeStyle}>{dateTime}</span>
        </div>
      );
        ...
        <div className={classes.bottomContainer}>
          <div className={classes.leftContainer}>
              ...
              {currentStockData && (
                <>
                  <div className={classes.price}>
                    ...
                    <StockPrice
                      oldPrice={currentStockData.base_price}
                      newPrice={generate}
                    />
                  </div>
                  <div>
            ...
            <div className={classes.leftbottom}>
              {currentStockData && (
                <StockGraph oldPrice={500} newPrice={generate} />
              )}
            </div>
          </div>
          ...
          <div style={{ width: "100%", marginTop: "5px" }}>
            ...
            <div className={classes.customDiv}>
              <p>
                <h3>Sagun brought 500 Zomato </h3> 10:00 AM
              </p>
              <p>
                <h3>Aakash sold 500 Zomato</h3> 10:00 AM
              </p>
              <p>
                <h3>Amey bought 500 Zomato </h3>10:00 AM
              </p>
            </div>
          </div>
        </div>
