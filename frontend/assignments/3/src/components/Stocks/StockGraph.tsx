...
export const StockGraph: React.FC<StockGraphProps> = ({
  ...
  const [data, setData] = useState<{ price: number; color: string }[]>([
    initialPrice,
  ]);
  ...
  useEffect(() => {
    if (newPrice) {
      const color =
        newPrice > data[data.length - 1].price ? "#b2f2bb" : "#ffc9c9";
  ...
  for (
    let i = 100;
    i <= Math.max(...data.map((item) => item.price));
    i += 100
  ) {
    gridLines.push(
      <div
        key={i}
        className="grid-line"
        style={{ borderLeft: "1px", height: `${i}px` }}
      />
    );
  }
