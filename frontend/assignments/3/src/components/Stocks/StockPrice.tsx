...
export function StockPrice({ oldPrice, newPrice }: StockPriceProps) {
  ...
  const [old, setOld] = useState<{ price: number; color: string }>(initialPrice);
  ...
  useEffect(() => {
    if (oldPrice !== undefined && newPrice !== undefined) {
      ...
      const percentageChange = parseInt((((old.price - newPrice) / old.price) * 100).toFixed(2));
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', width: '300px', height: '35px', alignContent: 'center', fontSize: '25px' }}>
      <div style={{marginRight:'10px'}}>
        Price: 
      </div>
      <div style={{ color: old.color}}>
        {old.price}
      </div>
